...
const foundersData = {
    ...
    9: {
        name: "Rojas Jiménez Alexa Michelle",
        role: "Encargada de control de calidad",
        image: "img/fundador9.jpg",
        bio: "Mi función es asegurar que cada producto que sea entregado al cliente tenga la mejor calidad, cumpliendo con las necesidades del cliente.",
...
/* Inicializador de carrusel genérico */
function initCarousel(carouselId, interval = 4000) {
    const container = document.getElementById(carouselId);
    if (!container) return;

    const track = container.querySelector('.carousel-track');
    const slides = Array.from(container.querySelectorAll('.carousel-image'));
...
// arrancar carruseles al cargar la página
document.addEventListener('DOMContentLoaded', function() {
    initCarousel('publicityCarousel', 5000);
});